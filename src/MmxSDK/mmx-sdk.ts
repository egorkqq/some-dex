...
import { getBaseUrl, hideWebsiteBelow } from "./helpers";
import {
  areUrlsEqual,
  importFonts,
...
export class MinimaxSDK {
  ...
  constructor(config: IHostConfig) {
    ...
    // пытаемся исправить косяки конфига от юзера, либо выкидываем ошибку
  ...
  // События, нужные СДК хендлятся здесь. Например, запрос на закрытие модалки, инит конфига
  ...
  // Embedded mode - встраиваем в DOM ноду
  ...
  // Overlay mode - поверх контента приложения клиента
  private _showUsingOverlayMode(): void {
    ...
    // Если вариант auto то внутри этой функции попытаемся определить лучший подходящий
